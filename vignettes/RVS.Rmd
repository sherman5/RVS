...
the individual sharing probabilties with *RVsharing*, but the sharing
probabilities can no longer be interpreted as a p-value. The function
...
The first step is reading in the data. See the *Data Input*
...
pedigree file in the LINKAGE format.
...
```{r}
result <- multipleVariantPValue(sample$genotypes, sample$fam, sharingProbs, filter_type='bonferroni', alpha=0.05)
```
...
the 4 most significant variants.

```{r}
freq <- seq(0.005,0.05,0.005)
variants <- names(sort(result$pvalues))[1:4]
...
plot(freq, pvals[,1], ylim=c(0,max(pvals)), type='l',
    xlab="minor allele frequency", ylab="p-value",
    main="sensitivity of p-value to allele frequency in four variants")
lines(freq, pvals[,2])
lines(freq, pvals[,3])
lines(freq, pvals[,4])
```
...
When founders of the pedigree are related, the computation is more tricky. *RVS*
allows the user to apply a correction for this fact in two different ways. The
first way, explained below, is a method from Bureau et al. (2014) that uses the
mean kinship coefficient among founders to apply a correction. For more details
on this calculation see [here](#exact-computation-with-kinship-coefficient).
*RVS* supports more complicated corrections using a Monte Carlo simulation
outlined [here](#using-monte-carlo-simulation).

```{r}
kin_coef <- seq(0.01, 0.1, length=5)
variants <- names(sort(result$pvalues))[1:4]
pvals <- matrix(nrow=length(kin_coef), ncol=length(variants))
for (i in 1:length(kin_coef))
{
...
plot(kin_coef, pvals[,1], ylim=c(0,max(pvals)), type='l',
    xlab="kinship coefficient", ylab="p-value",
    main="sensitivity of p-value to kinship in four variants")
lines(kin_coef, pvals[,2])
lines(kin_coef, pvals[,3])
lines(kin_coef, pvals[,4])
```

## Enrichment Test

Even if no individual variant has a significant p-value, it is possible
for all variants considered across all families to exhibit an unusual amount of
sharing. The *enrichmentPValue* function can compute a single p-value for all
variants seen in all families. When calculating this p-value, note that a very
...
# Analyzing variants in genomic sequence
