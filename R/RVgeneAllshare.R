...
RVgeneAllshare = function(ped.mat,ped.listfams,sites,fams,pshare.vec,type="alleles",minor.allele.vec,precomputed.prob=list(0),maxdim = 1e9)
{
	if (type=="alleles")
	{	
		if (missing(minor.allele.vec))
            minor.allele.vec = rep(2,length(sites))	
		...
		if (length(not)>0)
		{
    		if (2^nfam.info <= maxdim)
			{
                ...
                notShared = fam.info %in% not
    		  	...
    		  	pall = multipleFamilyPValue(fam.info,not,pshare)
			}
    		else 
            {
                pall = NA
