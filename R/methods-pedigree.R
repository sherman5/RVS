...
#' @export
setMethod("RVsharing", signature(data = "pedigree", dad.id = "missing",
mom.id = "missing", carriers = "missing"),
function(data)
{
    fa.index <- ifelse(data$findex != 0, data$findex, NA)
    fa.id <- data$id[fa.index]
...
#' @export
setMethod("RVsharing", signature(data = "character", dad.id = "character",
mom.id = "character", carriers = "character"),
function(data, dad.id, mom.id, carriers)
{
...
#' @export
setMethod("RVsharing", signature(data = "character", dad.id = "character",
mom.id = "character", carriers = "missing"),
function(data, dad.id, mom.id)
{
    # Converting 0s to NA  in dad.id and mom.id
    dad.id = ifelse(dad.id == "0", NA, dad.id)
    mom.id = ifelse(mom.id == "0", NA, mom.id)
...
#' @export
setMethod("RVsharing", signature(data = "numeric", dad.id = "numeric",
mom.id = "numeric", carriers = "numeric"),
function(data, dad.id, mom.id, carriers)
{
    # Converting 0s to NA  in dad.id and mom.id
    dad.id = ifelse(dad.id == 0, NA, dad.id)
...
#' @export
setMethod("RVsharing", signature(data = "numeric", dad.id = "numeric",
mom.id = "numeric", carriers = "missing"),
function(data, dad.id, mom.id)
{
    # Converting 0s to NA  in dad.id and mom.id
    dad.id = ifelse(dad.id == 0, NA, dad.id)
