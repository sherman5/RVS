...
infer.nalleles = function(phi,nf)
{
    ...
    term2 = 2*(a-nf)*2*(2*nf-a)/(2*(a-nf)*2*(2*nf-a) + 
        (a-nf)*(2*(a-nf)-1) + 2*(2*nf-a)*(2*nf-a-1))
...
{
    if (any(pshare$ped.tocompute.vec%in%vec))
    {
        sum(pshare$mlog10pshare[pshare$ped.tocompute.vec%in%vec])
    }
    else
    {
        NA
