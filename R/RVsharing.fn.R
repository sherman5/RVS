...
RVsharing.fn = function(id, dad.id, mom.id,carriers)
{
    ...
    {       
        ...
        list(fdi=fdi,ia=ia,lev.ia=lev.ia,iancestors=iancestors,iancestor.as.descendant=iancestor.as.descendant,
            desfounders=desfounders,foundersdegreedes=foundersdegreedes,ancestorsdegreedes=ancestorsdegreedes,spousevec=spousevec)
    }
                        ...
                        if (pl$ia>1)
                        {
                            ...
                            for (lia in 1:length(pl$lev.ia))
                            {
                                    ...
                                    if (!(cr %in% intersect(fd.subsets[[k]][,h],id[pl$fdi])))
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if (cr %in% union(dad.id[pl$fdi],mom.id[pl$fdi]))
                                            {
                                                ...
                                                # If he or she does not have at least one child among the current subset sharing a RV then add one of his children to the
                                                # carriers in his place and increment the meiosis reduction counter
                                ...
                                for (ian in 1:length(pl$lev.ia[[lia]]))
                                {
                                    ...
                                    # Check if all carriers in current possible subset are descendants of current intermediate ancestor and this is the first intermediate ancestors at this level,
                                    # or all final descendants are below the current intermediate ancestor, then the following intermediate ancestors
                            ...
                            for (cr in carriers)
                            {
                                ...
                                if (!(cr %in% intersect(fd.subsets[[k]][,h],id[fdi])))
                                {
                                    ...
                                    if (cr %in% union(dad.id[fdi],mom.id[fdi]))
                                    {
                                        ...
                                        # If he or she does not have at least one child among the current subset sharing a RV then add one of his children to the carriers in his place
                                        # and increment the meiosis reduction counter
                                        if (!any(fd.subsets[[k]][,h] %in% id[dad.id==cr | mom.id==cr]))
                                        {
